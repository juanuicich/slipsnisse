...
export class ClientManager {
  ...
  async init(mcpConfigs: Record<string, McpConfig>): Promise<void> {
    ...
    getLog().info({ connected, total: entries.length }, "MCP initialization complete");
  }

  private async connectServer(serverId: string, config: McpConfig): Promise<void> {
    getLog().debug({ serverId, command: config.command }, "Connecting to MCP server");

    try {
      const client = new Client({ name: `slipsnisse-${serverId}`, version: "1.0.0" });
      ...
      getLog().error({ serverId, error: (err as Error).message }, "Failed to connect to MCP server");
      this.connections.set(serverId, {
        serverId,
  ...
  async callTool(namespacedName: string, args: Record<string, unknown>): Promise<ToolCallResult> {
    ...
    const toolName = namespacedName.slice(sepIndex + NAMESPACE_SEPARATOR.length);
    ...
    if (conn.status !== "connected") {
      throw new Error(`MCP server unavailable: ${serverId} (status: ${conn.status})`);
    }
    ...
    for (const [serverId, conn] of this.connections) {
      if (conn.status === "connected") {
        try {
          ...
          getLog().error({ serverId, error: (err as Error).message }, "Error closing client");
        }
      }
    }
