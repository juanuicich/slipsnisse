...
const mockedMethods = vi.hoisted(() => ({
    ...
    mockListTools: vi.fn().mockResolvedValue({ tools: [{ name: "tool1", description: "desc1", inputSchema: {} }] }),
    mockCallTool: vi.fn().mockResolvedValue({ content: [{ type: "text", text: "result" }], isError: false }),
...
describe("ClientManager", () => {
	...
	beforeEach(() => {
        ...
        mockedMethods.mockListTools.mockResolvedValue({ tools: [{ name: "tool1", description: "desc1", inputSchema: {} }] });
        mockedMethods.mockCallTool.mockResolvedValue({ content: [{ type: "text", text: "result" }], isError: false });
